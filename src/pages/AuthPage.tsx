...
export function AuthPage() {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex flex-col items-center justify-center h-screen p-4 sm:p-8 crt-screen"
    ...
    >
      <div className="w-full max-w-md z-10 flex flex-col items-center">
        ...
        <h2 className="font-display text-sm md:text-base text-center text-amber-300 mb-10 tracking-[0.3em] opacity-80">
          ROGUELIKE TRAINER
        </h2>

        <Card
          className="w-full p-8 md:p-10 border-4 border-border bg-surface-alt flex flex-col gap-8"
          noPadding={false}
        >
          <div className="flex flex-col gap-2">
            <h3 className="font-display text-xs md:text-sm text-brand text-subtitle tracking-widest drop-shadow-[0_0_5px_rgba(255,215,0,0.3)]">
              {mode === "login" ? "INICIAR SESIÓN" : "CREAR CUENTA"}
            </h3>
            <p className="font-body text-xs text-subtitle">
              {mode === "login"
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            <div className="flex flex-col gap-3">
              <label className="font-display text-[1rem] text-muted-foreground uppercase tracking-widest">
                Email
              </label>
              <input
                type="email"
          ...
          <div className="relative h-4 flex items-center justify-center">
            ...
            <span className="relative bg-surface-alt px-4 font-display text-xs text-muted uppercase">
              o
            </span>
          </div>
          ...
          >
            ...
            <span className="font-display text-[1rem]">GOOGLE</span>
          ...
          <button
            type="button"
            onClick={toggleMode}
            className="text-center font-display text-[0.8rem] text-brand hover:text-accent transition-colors uppercase tracking-widest"
          >
            {mode === "login"
          ...
          <div className="border-t border-border pt-6 mt-2 flex flex-col gap-4">
            <p className="font-body text-[0.7rem] text-muted-foreground text-center px-4 leading-relaxed">
              <span className="text-accent underline">MODO INVITADO:</span>{" "}
              Juega al instante. Tus datos <strong>NO persistirán</strong> al
              cerrar el navegador.
            </p>
            <Button
              type="button"
              ...
              className="w-full text-xs border-2 mt-4 border-border opacity-80 hover:opacity-100"

            >
              <span className="font-display text-[1rem]">ENTRAR COMO INVITADO</span>
        ...
        message={
          <div className="flex flex-col gap-4">
            ...
            <p className="text-warning bg-warning/10 p-3 border border-warning/20">
              Tu progreso <strong>SOLO</strong> se guardará en la sesión actual.
              Si cierras la pestaña o el navegador, perderás toda tu aventura.
            </p>
            <p>
              Te recomendamos crear una cuenta para guardar tu progreso en la
