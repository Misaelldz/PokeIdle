...
export function MoveManager({ pokemon }: Props) {
    ...
    setRun((prev) => ({
      ...
      team: prev.team.map((p) => {
        ...
        return {
          ...p,
          moves: p.moves.map((m) =>
            m.moveId === moveId ? { ...m, enabled: !m.enabled } : m,
          ),
        };
      }),
    }));
  ...
  return (
    <div className="flex flex-col gap-1.5 mt-2">
      <h4 className="text-[0.6rem] font-display text-muted uppercase">
        Movimientos
      </h4>
      <div className="grid grid-cols-1 gap-1.5">
        {pokemon.moves.map((move: any) => (
          <button
            ...
            title={
              move.enabled ? "Desactivar movimiento" : "Activar movimiento"
            }
          >
            <div className="flex flex-col gap-0.5 max-w-[70%]">
              <span className="font-display text-[0.55rem] text-foreground truncate">
                {move.moveName}
              </span>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[0.6rem] font-body text-muted shrink-0">
                  PP: {move.currentPP}
                </span>
              </div>
            </div>
            <div className="text-[0.6rem] font-body text-muted shrink-0 text-right">
              {move.power > 0 ? `PDR: ${move.power}` : "--"}
            </div>
          </button>
        ))}
        {pokemon.moves.length === 0 && (
          <div className="text-xs text-muted font-body italic">
            Sin movimientos
          </div>
        )}
      </div>
