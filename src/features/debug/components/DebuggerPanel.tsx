  ...
  return (
    <div className="fixed bottom-4 left-4 z-50 w-[350px] max-h-[500px] flex flex-col bg-surface-dark border-4 border-brand pixel-shadow overflow-hidden font-display text-[0.6rem]">
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 bg-surface custom-scrollbar">
        ...
        {activeTab === 'pokemon' && (
          <div className="flex flex-col items-center justify-center p-8 gap-4 opacity-50 italic text-center">
            <Package size={32} />
            <span>Funcionalidad de inyección de Pokémon en desarrollo...</span>
          </div>
        )}
      </div>

      <div className="p-2 bg-surface-alt border-t border-border flex justify-center">
         <span className="text-[0.45rem] text-muted tracking-tight text-center uppercase">Dev Toolbar - use responsibly</span>
      </div>
    </div>
  );
}
